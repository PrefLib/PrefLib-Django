...
urlpatterns = [
    ...
    re_path(r'^data/(?P<datacategory>' + '|'.join(map(lambda x: x[0], DATACATEGORY)) + ')[/]?$', views.alldatasets, name = 'alldatasets'),
    re_path(r'^data/(?P<datacategory>' + '|'.join(map(lambda x: x[0], DATACATEGORY)) + ')/(?P<dataSetNum>[0-9]{5})[/]?$', views.dataset, name = 'dataset'),
    re_path(r'^data/(?P<datacategory>' + '|'.join(map(lambda x: x[0], DATACATEGORY)) + ')/(?P<dataSetNum>[0-9]{5})/(?P<dataPatchNum>[0-9]{8})[/]?$', views.datapatch, name = 'datapatch'),
    ...
    re_path(r'^profile[/]?$', views.userProfile, name = 'userProfile'),
